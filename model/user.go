...
type User struct {
	...
	ActiveEnterpriseId int                        `json:"-" gorm:"type:int;not null;default:0;column:active_enterprise_id;index"`
	...
	if err := DB.Transaction(func(tx *gorm.DB) error {
		return withNormalizedEmailLock(tx, user.Email, func(tx *gorm.DB) error {
			...
			if isPlatformAdminRole(user.Role) {
				return EnsurePlatformAdminEnterpriseWithTx(tx, user)
			}
			return nil
		})
	}); err != nil {
